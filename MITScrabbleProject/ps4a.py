...
def getWordScore(word,n):
    ...
    for i in range(len(word)):
        sum = sum + SCRABBLE_LETTER_VALUES[word[i]]
    i = n - len(word)
    if(i==0):
        sum = sum*n + 50
    else:
...
def playHand(hand, wordList, n):
   ...
   # displayHand(hand)
    ...
    total = 0
    size = calculateHandlen(handcopy)
    l=[]
    while(calculateHandlen(handcopy)!=0):
        ...
        if word==".":
            break;
        elif(isValidWord(word, handcopy, wordList) == False):
            print 'Invalid word,please try again.'
        else:
            pointsEarned = getWordScore(word, calculateHandlen(hand))
            total = total + pointsEarned
            print "\"" + word + "\" earned", pointsEarned, " points. Total:", total, "points" 
...
def playGame(wordList):
    ...
    lastHand = []
    while playing == True: 
        menuChoice = raw_input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
        if menuChoice == "n":
...
if __name__ == '__main__':
    wordList = loadWords()
    playGame(wordList)
    #print(getWordScore('tea',7))
  #  hand = {'a':1, 'q':1, 'l':2, 'm':1, 'u':1, 'i':1}
    #hand = {'y': 1, 'x': 2, 's': 1, 'z': 2, 'b': 1}
    hand = {'a': 2, 'p': 2, 'r': 1, 'z': 1, 'e': 1}
   # hand = {'a': 2, 'p': 1, 'r': 1, 'e': 2, 't': 1}
    #print(playHand(hand, wordList, 7))

    
  #  print(updateHand(hand,'quail'))

#print(isValidWord('l',hand,wordList))
